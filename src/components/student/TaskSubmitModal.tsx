...
interface TaskSubmitModalProps {
  task: (Task & { assignment_id?: number; assignment_status?: string; submission_response?: string; submission_file_url?: string; submission_file_name?: string }) | null;
  isOpen: boolean;
  onClose: () => void;
      ...
      <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="p-6 border-b border-slate-200 bg-slate-50/50 flex items-center justify-between">
          <div className="space-y-1">
            ...
            <h3 className="font-bold text-slate-900 text-base leading-tight mt-1">{task.title}</h3>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-4 flex-1 text-sm">
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
            <div className="text-xs font-bold text-slate-500 uppercase">Instructions & Details</div>
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <span className="text-xs font-bold">YES, I COMPLETED IT</span>
                </button>
                ...
                >
                  ...
                  <span className="text-xs font-bold">NO, NOT YET</span>
                </button>
              </div>
            </div>
          )}
          ...
          {task.type === 'FILE_SUBMISSION' && (
            <div className="space-y-3 pt-2">
              <label className="block text-xs font-bold text-slate-700">
                Upload Required Proof Document (PDF, PNG, JPG, DOCX - Max 15MB) *
              </label>

              <div className="border-2 border-dashed border-slate-300 hover:border-blue-500 rounded-2xl p-6 text-center transition-colors bg-slate-50 relative cursor-pointer">
                <input
                  type="file"
                ...
                <div className="font-bold text-slate-800 text-xs">
                  {uploading ? 'Uploading...' : uploadedFile ? uploadedFile.name : 'Click to Upload Proof Document'}
                </div>
                <p className="text-[11px] text-slate-500 mt-1">
                  {uploadedFile ? `${formatFileSize(uploadedFile.size)} • Ready to submit` : 'PDF, JPG, PNG, DOCX up to 15MB'}
                </p>
              </div>
            ...
            <div className="space-y-4 pt-2">
              {task.external_link && (
                <a
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 p-3.5 rounded-xl font-bold text-xs transition-colors"
                >
              ...
              <label className="flex items-start gap-2.5 p-3.5 rounded-xl border border-slate-200 bg-slate-50 cursor-pointer">
                <input
                  type="checkbox"
                ...
                <span className="text-xs font-semibold text-slate-800 leading-snug">
                  I have opened the external link and completed the entire required activity.
                </span>
              </label>
            </div>
          )}
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50 flex items-center justify-end gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-6 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white text-xs font-bold rounded-xl shadow-xs"
          >
